...
class ScanApprovalRequest(BaseModel):
    ...
    approved: bool = Field(..., description="Whether to approve active/intrusive tests")
...
async def create_scan(body: ScanCreateRequest, request: Request) -> ScanCreateResponse:
    ...
    orchestrator = get_orchestrator()
    try:
        orchestrator.register_scan(scan_id, body.target)
        task = orchestrator.schedule_scan(scan_id, body.target)
    except Exception:
        await _scan_rate_limiter.release(client_id)
        raise

    def _release_slot(_task: asyncio.Task[None]) -> None:
        asyncio.create_task(_scan_rate_limiter.release(client_id))

    task.add_done_callback(_release_slot)

    logger.info("Scan queued", extra={"scan_id": scan_id, "target": body.target})
...
async def get_scan_status(scan_id: str, target: str | None = None) -> ScanStatusResponse:
    orchestrator = get_orchestrator()
...
async def approve_scan(scan_id: str, body: ScanApprovalRequest) -> ScanApprovalResponse:
    orchestrator = get_orchestrator()
    ...
    if body.target is not None:
        enforce_scope(body.target)
        if registered_target != body.target:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail={"error": "scan_not_found", "scan_id": scan_id},
            )
    ...
    await orchestrator.resume_scan(scan_id, approved=body.approved)
    updated = await orchestrator.get_status_summary(scan_id)
    ...
    return ScanApprovalResponse(
        ...
        human_approved=updated["human_approved"],
        is_complete=updated["is_complete"],
    )


@app.get("/scan/{scan_id}/report", response_model=ScanReportResponse)
async def get_scan_report(scan_id: str, target: str | None = None) -> ScanReportResponse:
    orchestrator = get_orchestrator()
    summary = await orchestrator.get_status_summary(scan_id)
    if summary is None:
        raise HTTPException(
...
async def get_scan_report_format(
    ...
    if report_format not in {"json", "md", "html"}:
        raise HTTPException(
            ...
            detail={
                ...
                "message": "Supported formats: json, md, html",
            },
        )
    ...
    if effective_target != summary["target"]:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail={"error": "scan_not_found", "scan_id": scan_id},
        )
